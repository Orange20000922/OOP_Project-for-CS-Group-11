...
\textbf{Usage}: \texttt{AuthService.sessions} maps session tokens to \texttt{SessionRecord} objects for $O(1)$ authentication on every HTTP request. \texttt{UserStore.users} provides $O(1)$ student ID lookups. \texttt{ScheduleService.tasks} tracks asynchronous scraper task status.

\textbf{Rationale}: Session validation occurs on every API call, making $O(1)$ average-case lookup essential. The hash table's dynamic resizing handles variable numbers of concurrent sessions gracefully. We rejected a Python \texttt{dict} because the course explicitly requires hand-implemented structures.
...
\subsubsection{Stack}
Linked-list-based LIFO structure used in the knowledge tree module for depth-first traversal of topic hierarchies when computing subtree aggregations and validating parent-child relationships during topic CRUD operations.
...
Beyond meeting course requirements, the system addresses a real student need: managing university schedules with live data integration and AI-assisted learning tools. The three-tier SCNU portal integration, secure credential handling, and knowledge workspace with RAG capabilities represent engineering effort beyond typical course project scope. The layered architecture, comprehensive test suite, and structured logging reflect professional software development practices.

The result is a functional prototype usable in daily student life, while serving as a demonstration of OOP design, algorithmic thinking, and iterative development informed by testing and real-world constraints.
